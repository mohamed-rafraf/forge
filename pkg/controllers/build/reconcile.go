...
import (
	...
	"github.com/forge-build/forge/pkg/external"
...
func Add(ctx context.Context, mgr ctrl.Manager, numWorkers int, log logr.Logger, options controller.Options) error {
	...
	reconciler.externalTracker = external.ObjectTracker{
...
func (r *BuildReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	if kubernetes.IsPaused(build, build) {
...
func (r *BuildReconciler) reconcileDelete(ctx context.Context, build *buildv1.Build) (reconcile.Result, error) {
	...
	if err != nil {
		r.Logger.Error(err, "Failed to list descendants")
		return reconcile.Result{}, err
	}

	children, err := descendants.filterOwnedDescendants(build)
	if err != nil {
		r.Logger.Error(err, "Failed to extract direct descendants")
		return reconcile.Result{}, err
	}

	if len(children) > 0 {
		r.Logger.Info("Build still has children - deleting them first", "count", len(children))
		...
		for _, child := range children {
			...
			log := r.Logger.WithValues(gvk.Kind, klog.KObj(child))
	...
	if descendantCount := descendants.length(); descendantCount > 0 {
		...
		r.Logger.Info("Build still has descendants - need to requeue", "descendants", descendants.descendantNames(), "indirect descendants count", indirect)
	...
	if build.Spec.InfrastructureRef != nil {
		...
		default:
			...
			r.Logger.Info("Build still has descendants - need to requeue", "InfrastructureRef", build.Spec.InfrastructureRef.Name)
			return ctrl.Result{}, nil
		}
	}
...
func (r *BuildReconciler) reconcileExternal(ctx context.Context, build *buildv1.Build, ref *corev1.ObjectReference) (external.ReconcileOutput, error) {
	if err := kubernetes.UpdateReferenceAPIContract(ctx, r.Client, ref); err != nil {
	...
	if err != nil {
		if apierrors.IsNotFound(errors.Cause(err)) {
			r.Logger.Info("Could not find external object for build, requeuing", "refGroupVersionKind", ref.GroupVersionKind(), "refName", ref.Name)
	...
	if err := r.externalTracker.Watch(r.Logger, obj, handler.EnqueueRequestForOwner(r.Client.Scheme(), r.Client.RESTMapper(), &buildv1.Build{})); err != nil {
		return external.ReconcileOutput{}, err
	}
