...
func TestCloneTemplateResourceFound(t *testing.T) {
	...
	ref, err := CreateFromTemplate(ctx, &CreateFromTemplateInput{
		...
		Labels: map[string]string{
			"precedence":           "input",
			buildv1.BuildNameLabel: "should-be-overwritten",
		},
		Annotations: map[string]string{
			"precedence": "input",
			//buildv1.TemplateClonedFromNameAnnotation: "should-be-overwritten",
		},
	})
	g.Expect(err).ToNot(HaveOccurred())
	...
	g.Expect(cloneLabels).To(HaveKeyWithValue(buildv1.BuildNameLabel, testClusterName))
...
func TestCloneTemplateResourceFoundNoOwner(t *testing.T) {
	...
	expectedKind := "Yellow"
	expectedAPIVersion := templateAPIVersion
	expectedLabels := map[string]string{buildv1.BuildNameLabel: testClusterName}
